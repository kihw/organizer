...
;**************************************************************************************************
; Menus.au3
; Source: ModernMenu UDF by Holger Kotsch
; Version-date: 06.05.2008
; Updated by ZDS (31.05.2018)
;**************************************************************************************************

#include-once

; Affichage d'un sousmenu
Func sousmenu($gui, $target, $context)
	Local $handle = $gui, $menu = GUICtrlGetHandle($context), $coords = DllStructCreate("int;int")
	If $gui = 0 Then
		$handle = GUICreate("", 1, 1, 0, 0, 0x80880000, 0x00000080)
		WinSetTrans($handle, "", 0)
		GUISetState(@SW_SHOW)
		WinActivate($handle)
	EndIf
	If IsArray($target) Then
		DllStructSetData($coords, 1, $target[0])
		DllStructSetData($coords, 2, $target[1])
	ElseIf $target = 0 Then
		DllStructSetData($coords, 1, MouseGetPos(0))
		DllStructSetData($coords, 2, MouseGetPos(1))
	Else
		Local $position = ControlGetPos($handle, "", $target)
	...
	EndIf
	DllCall("user32.dll", "int", "TrackPopupMenuEx", "hwnd", $menu, "int", 0, "int", DllStructGetData($coords, 1), "int", DllStructGetData($coords, 2), "hwnd", $handle, "ptr", 0)
	If $gui = 0 Then
		GUIDelete($handle)
	EndIf
EndFunc
...
; ID/Text/IconIndex/ParentMenu/Tray/SelIconIndex/IsMenu/Style
Global $arMenuItems[1][9]
$arMenuItems[0][0] = 0
...
;********************************************************************
; Set the style of an menu item
;********************************************************************
Func _TrayItemSetStyle($nMenuID = -1, $sStyle = 0)
	If $nMenuID = -1 Then $nMenuID = $TRAYLASTITEM
	If $nMenuID <= 0 Then Return 0
...
Func _GetNewItemIndex()
	...
	If Not $bFreeFound Then
		...
		If $arMenuItems[0][0] > $nSize - $nMenuItemsRedim Then _
			Redim $arMenuItems[$nSize + $nMenuItemsRedim][Ubound($arMenuItems,2)]
		$i = $arMenuItems[0][0]
	EndIf
...
; Set the style of an menu item
;********************************************************************
Func _GUICtrlODMenuItemSetStyle($nMenuID, $sStyle)
	If $nMenuID = -1 Then $nMenuID = $MENULASTITEM
	If $nMenuID <= 0 Then Return 0
